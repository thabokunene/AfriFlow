...
class ProducerConfig:
    ...
    client_id: str = "afriflow-pbb-producer"
    ...
    dlq_topic: Optional[str] = "pbb.dlq"
    ...
    def from_env(cls, env: Optional[str] = None) -> 'ProducerConfig':
        ...
        return cls(
            ...
            client_id=f"afriflow-pbb-producer-{environment}",
            acks=acks,
...
class PBBKafkaProducer:
    """
    High-performance Kafka producer for PBB (Personal & Business Banking) events.
    ...
    # Topic names
    ACCOUNT_TOPIC = "pbb.accounts"
    PAYROLL_TOPIC = "pbb.payroll"
    ...
    def __init__(
        ...
        self.logger = get_logger("pbb.ingestion.kafka_producer")
        ...
        try:
            ...
            self.logger.info(
                f"PBBKafkaProducer initialized for {self.bootstrap_servers}"
            )
    ...
    def produce_account(
        ...
        """
        Produce a PBB account event.

        Args:
            account_data: Dictionary containing account data.
            sync: If True, blocks until message is delivered.
            key: Optional message key for partitioning (defaults to account_id).
            
        Example:
            >>> producer = PBBKafkaProducer()
            >>> producer.produce_account({
        ...
        if key is None:
            key = account_data.get("account_id")
        self._produce_event(topic, "account_v1", account_data, sync, key)
    ...
    def produce_payroll(
        ...
        """
        Produce a PBB payroll event.

        Args:
            payroll_data: Dictionary containing payroll data.
            sync: If True, blocks until message is delivered.
            key: Optional message key for partitioning (defaults to payroll_batch_id).
            
        Example:
            >>> producer = PBBKafkaProducer()
            >>> producer.produce_payroll({
            ...
            ... })
        """
        topic = self.PAYROLL_TOPIC
        if key is None:
            key = payroll_data.get("payroll_batch_id")
        self._produce_event(topic, "payroll_v1", payroll_data, sync, key)
...
def create_producer(
    ...
    """
    Create and return a configured PBBKafkaProducer.
    ...
    Returns:
        Configured PBBKafkaProducer instance.
    """
    ...
    return PBBKafkaProducer(config=config)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="PBB Kafka Producer")
    ...
    if args.demo:
        ...
        try:
            ...
            producer.produce_account(account_data)
            print(f"Account sent: {account_data['account_id']}")
            
            # Send demo payroll
            payroll_data = {
                "payroll_batch_id": "DEMO-PAY-001",
                "employer_id": "DEMO-EMP-001",
            ...
            producer.produce_payroll(payroll_data)
            print(f"Payroll sent: {payroll_data['payroll_batch_id']}")
        ...
        print("PBB Kafka Producer ready.")
