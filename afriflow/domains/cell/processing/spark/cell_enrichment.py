...
@description Spark-style enrichment wrapper for Cell domain data, enforcing RBAC and input validation.
@author Thabo Kunene
@created 2026-03-19
"""

# Standard logging for operational observability and error tracking
import logging
# BaseProcessor defines the minimal processing lifecycle used across all domains
from afriflow.domains.shared.interfaces import BaseProcessor
# get_config provides environment-aware settings for RBAC and operational limits
from afriflow.domains.shared.config import get_config


class Processor(BaseProcessor):
    """
    Enrichment processor for the Cell domain.
    Ensures that data transformation follows strict security and structural rules.
    ...
    def process_sync(self, record):
        """
        Synchronously enriches a cell domain record after validation.
        
        :param record: The input record to enrich.
        :return: A copy of the record with an enrichment marker.
        ...
        try:
            ...
            out = dict(record)
            # Flag record as having passed through the enrichment layer
            out["processed"] = True
            return out
        except Exception as e:
            # Log the error with structured metadata for log aggregation systems
